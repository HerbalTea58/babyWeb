...
<title>修改文章</title>
	<link rel="stylesheet" href="<%=basePath%>/css/maFindEssay.css" type="text/css" />
</head>
<body>
	<div class="top">
	...
	<div class="main">
		<form action="<%=basePath %>/manage/updateEssay.do" method="post">
			<input name="mid" style="display:none" value="${mid}"/>
			<input name="tid" style="display:none" value="${essay.tid}"/>
			<span class="title">标题: </span>
			<input class="title1" name="title" value="${essay.title }"/>
		
			<span class="preread">导读: </span>
			<textarea rows="6" cols="70" class="preread1" name="preread">${essay.preread}</textarea>
			...
			<span class="con">正文:</span>
			 ...
			 <div id="but2" class="but2">
				<div id="div1">${essay.ecomtent }</div>
			 </div> 
			 <textarea id="text1" style="display:none" name="con"></textarea>
			
			<button type="submit" class="but3">修改</button>
		</form>
	</div>
